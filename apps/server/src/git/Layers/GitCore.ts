...
const STATUS_UPSTREAM_REFRESH_FAILURE_BACKOFF = Duration.seconds(60);
...
const makeGitCore = Effect.gen(function* () {
  ...
  const statusUpstreamRefreshCache = yield* Cache.makeWith({
    ...
    // Keep successful refreshes warm; back off on failures to avoid spamming slow/unreachable remotes.
    timeToLive: (exit) =>
      Exit.isSuccess(exit)
        ? STATUS_UPSTREAM_REFRESH_INTERVAL
        : STATUS_UPSTREAM_REFRESH_FAILURE_BACKOFF,
  });
