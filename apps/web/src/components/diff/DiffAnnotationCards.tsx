      ...
      <div className="flex items-center gap-1">
        ...
        {isPublished ? (
          ...
          <button
            ...
            disabled={annotation.isPublishing}
            className="inline-flex h-4 items-center gap-1 rounded border border-border/50 bg-background/50 px-1.5 text-[10px] text-muted-foreground/70 transition-colors hover:border-border hover:bg-foreground/10 hover:text-foreground/80 disabled:opacity-50 disabled:cursor-not-allowed"
            title="Publish this comment to GitHub"
          >
            <GitHubIcon className="size-2.5 shrink-0" />
            <span>{annotation.isPublishing ? "Publishing..." : "Publish"}</span>
          </button>
        ) : null}
      </div>
