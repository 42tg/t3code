 ...
 * on top of `@pierre/diffs` file diffs. Each annotation kind is a
 * discriminated union variant so the renderer can dispatch to kind-specific
 * cards while all pipeline plumbing (path normalisation, line mapping,
 * grouping) stays shared.
 *
 * To add a new annotation kind:
 *   1. Add a new interface to the DiffAnnotation union below.
 *   2. Add a case in `renderDiffAnnotation` (DiffAnnotationCards.tsx).
 */
 ...
 *
 * Currently empty (placeholder) -- concrete kinds (review comments, lint
 * warnings, AI suggestions, ...) are added by downstream features.
 */
export type DiffAnnotation = PlaceholderAnnotation | ReviewCommentAnnotation;

/** Placeholder so the union is never empty. Filtered out at render time. */
...
/** A review comment created by the review_comment MCP tool. */
...
export function toDiffLineAnnotations(
  ...
  return annotations.map((annotation) => ({
    ...
    metadata: annotation as DiffAnnotation,
