...
export function useReviewAnnotations(
  ...
  const pendingPublishIds = useRef(new Set<string>());

  const onPublish = useCallback(
    (commentId: string) => {
      if (!threadId || !cwd || !prUrl) return;
      if (pendingPublishIds.current.has(commentId)) return;
      pendingPublishIds.current.add(commentId);
      publishMutation.mutate(
        { threadId, cwd, prUrl, commentId },
        {
          ...
          onSettled: () => {
            pendingPublishIds.current.delete(commentId);
          },
        },
      );
    },
  ...
  return list.map((c): ReviewCommentAnnotation => {
    ...
    if (prUrl) {
      annotation.onPublish = onPublish;
      annotation.isPublishing = pendingPublishIds.current.has(c.id);
    }
    return annotation;
  });
}
