...
export function useReviewAnnotations(
  ...
  const publishingIds = useRef(new Set<string>());

  const onPublish = useCallback(
    (commentId: string) => {
      if (!threadId || !cwd || !prUrl) return;
      if (publishingIds.current.has(commentId)) return;
      publishingIds.current.add(commentId);
      publishMutation.mutate(
        { threadId, cwd, prUrl, commentId },
        {
          ...
          onSettled: () => {
            publishingIds.current.delete(commentId);
          },
        },
      );
    },
  ...
  return list.map((c): ReviewCommentAnnotation => {
    ...
    if (prUrl) {
      ...
      annotation.isPublishing = publishingIds.current.has(c.id);
    }
    return annotation;
  });
}
